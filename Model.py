...
class Transformer(nn.Module):
    # 修改 __init__ 增加 backbone 参数
    def __init__(self, input_shape, output_dim=2, d_model=128, n_heads=2, num_layers=2, dropout=0.1, extra_dim=0, backbone=None):
        super().__init__()
        len_window, feature_dim = input_shape
        self.d_model = d_model
        
        # 如果传入了 backbone，则共享其权重；否则创建新层
        if backbone is not None:
            self.input_proj = backbone.input_proj
            # 注意：pos_encoding 是 buffer，虽然内容固定，这里直接引用即可
    ...
    def forward(self, x, extra_features=None):
        ...
        x = x.to(device).float()
        x = self.input_proj(x)
        x = x + self.pos_encoding[:, : x.size(1)].to(device)
